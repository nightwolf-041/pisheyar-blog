              ...
              `,
                feeds: [{
                    serialize: ({ query: { site, allRestApiPostGetAllAnonymous } }) => {
                        const configPosts = data.allRestApiPostGetAllAnonymous.edges
                        let posts = [...graphPosts[0].node.posts]
                        return allRestApiPostGetAllAnonymous.edges.map(edge => {
                            return Object.assign({}, edge.node.posts, {
                                description: edge.node.posts.abstract,
                                date: edge.node.posts.modifiedDate,
                                url: site.siteMetadata.siteUrl + edge.node.posts.slug,
                                guid: site.siteMetadata.siteUrl + edge.node.posts.slug,
                                custom_elements: [{ "content:encoded": edge.node.posts.description }],
                            })
                        })
                    },
                    query: `
