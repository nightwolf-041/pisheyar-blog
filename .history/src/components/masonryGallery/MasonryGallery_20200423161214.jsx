...
const MasonryGallery = () => {
    ...
    let currentPosts = posts.slice((1 - 1) * 12, 1 * 12);

    let [currentPage, setCurrentPage] = useState(1)
    let [pagesIndex, setpagesIndex] = useState(12)

    useEffect(() => {
        return () => currentPosts.slice((currentPage - 1) * pagesIndex, currentPage * pagesIndex);
    }, currentPage)

    const aginationChangeHandler = (current, pageIndex) =>{
        console.log(`current is ${current}`);
        setCurrentPage(current)
        setpagesIndex(pageIndex)
        console.log(currentPosts.slice((current - 1) * pageIndex, current * pageIndex));
    }
    ...
    return(
        ...
        <Pagination
            ...
            current={currentPage}
            defaultPageSize={12}
            onChange={(current, pageIndex) => aginationChangeHandler(current, pageIndex)}
        />
        </>
    )
}
